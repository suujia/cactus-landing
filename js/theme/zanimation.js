...
const filterBlur = () => {
  let blur = 'blur(5px)';
  const isIOS = window.is.iphone() || window.is.ipad() || window.is.ipod() || window.is.mac();
...
const breakPointConst = utils.getCurrentScreanBreakpoint();
      ...
      if (value.hasAttribute(`data-zanim-${breakPointConst.currentScrean}`)) {
        ...
        $.each(value.attributes, (i, attribute) => {
          if (attribute.name !== 'data-zanim-trigger' && ((window.is.ie() || window.is.edge()) ? attribute.name.match('^data-zanim-') : attribute.name.startsWith('data-zanim-'))) {
            const breakPoint = utils.breakpoints[attribute.name.split('data-zanim-')[1]];
